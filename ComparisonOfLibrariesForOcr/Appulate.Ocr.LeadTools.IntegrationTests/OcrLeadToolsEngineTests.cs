...
using System.IO;
using System.Linq;
using Appulate.Ocr.LeadTools.Core;
using FluentAssertions;
using NUnit.Framework;

namespace Appulate.Ocr.LeadTools.IntegrationTests {
	[TestFixture]
	public class OcrLeadToolsEngineTests {
		[Test]
		public void RecognizesAcord130UsingAnOcrTemplate() {
			...
			s1.Start();
			var engine = new LeadToolsEngine(templatePath);
			s1.Stop();
			Console.WriteLine($"first initialization: {s1.Elapsed.Seconds} seconds");

			var s2 = new Stopwatch();
			s2.Start();
			List<(int Page, string Field, string Text)> result = engine.Run(filePath);
			s2.Stop();
			...
			items[effectiveDataFieldId].Trim().Should().Be("07/17/2017");
			items[expirationDataFieldId].Trim().Should().Be("07/17/2018");
		}
	}
}